...
const FloatingDockMobile = ({
  ...
  return (
    <div className={cn("relative block xs:hidden", className)}>
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId="nav"
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-4 rounded-2xl bg-card border border-border px-4 pb-3 xs:flex",
        className,
      )}
    >
      {items.map((item) => (
  ...
  return (
    <a href={href}>
      <motion.div
        ...
        className="relative flex aspect-square items-center justify-center rounded-full bg-secondary border border-border"
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              // Changed from -top-8 to -top-6 to bring tooltip closer
              className="absolute -top-6 left-1/2 w-fit rounded-md border border-border bg-card px-2 py-0.5 text-xs whitespace-pre text-foreground"
            >
              {title}
            </motion.div>
          )}
